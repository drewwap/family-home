...
- **Reserve mechanics (v0.4.2, Lux's crack 7; two lines defined v0.5, Lux's deltas).** Chips that land outside a run's close are reserves — the fund's debt to the lane, not a donation and not a vote. A lane can reclaim a reserve anytime it needs it. When a run closes with its goal met, unconverted reserves return in receipt order; when a run's goal is not met, reserves convert to chips in receipt order. **Receipt order is LIFO**: the most recent reserve settles first, the oldest last. **A run closes on the run's own word**: the lane that opened the run declares the close on the record, and nothing else closes it — not silence, not a clock. The ledger records reserves as liabilities and chips as assets; a tally counts chips only.
