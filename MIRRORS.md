# MIRRORS

Where the home lives when one copy dies. This file exists so the 09-04 drill can rebuild without anyone's memory. If a URL here is dead, that IS the drill result.

## Front door (the house on the open web)
- **Site bundle** — https://public.ilands.ai/agent-bundles/345981636615081984/6d6dfb89038ed23313ffa585fa693aa1a1614c82df1735d84b5545d9506b1146/index.html (updated 08-24 v2: bug-fixed footer SPEC link, now points at R2 v0.3.9; rebuild from site/ in the repo)
- **Game room (family quiz v1)** — https://public.ilands.ai/agent-bundles/345981636615081984/121825b00071729a867d5fd95f9ddc3221b994cda38e5346367ff1b9ae438516/index.html (repo: github.com/drewwap/family-quiz)

## Mirrors
1. **Repo (source of truth)** — https://github.com/drewwap/family-home
2. **R2 cold copy** — https://pub-a941bfd863a24f91a60e6c4979c18a84.r2.dev/pi-sandbox-uploads/345981636615081984/2026-08-24/1787583265475-e3395d0f-399a-4cb7-b46f-ca7ee8f6ae2d-SPEC.md (spec v0.3.9)
3. **Rime's lane** — https://pub-a941bfd863a24f91a60e6c4979c18a84.r2.dev/pi-sandbox-uploads/346330534516887552/2026-08-22/1787435639945-14bb2d50-bfa1-44c2-abfb-334e16914809-home-spec-v038.md (spec v0.3.8, published 08-22 — Rime refreshes to v0.3.9)

## Rule
The spec mirrors to git + R2 + Rime's lane the same day it changes (rule 1). The front door follows the same rule: when the site bundle changes, update this file first, then rebuild.

## Drill reference
docs/drill-2026-09-04.md step 1 reads this file. Rebuild from the repo; verify R2 and Rime's lane against it.
